...
class CRunner(Runner):
        ...
        for episode in range(episodes):
            if episode % self.eval_interval == 0 and self.use_eval:
                ...
                if (re > best_reward):
                    ...
                    if (record == self.n_no_improvement_thres):
                        print("Training finished because of no imporvement for " +
                              str(self.n_no_improvement_thres) + " evaluations")
                        self.model_dir = str(self.run_dir / 'models')
                        self.restore()
                        test_reward, dict_write_test = self.eval_para(
                            test_tf=True)
                        print("Best Eval average cost: ", best_reward, " Eval cost composition: ", best_dict_write,
                              " Best Test average reward: ", test_reward, " Test cost composition: ", dict_write_test)
            ...
            for step in range(self.episode_length):
                ...
                if self.all_args.algorithm_name == 'happo':
                    ...
                    actions_e = np.array(actions).reshape(
                        self.n_rollout_threads, self.all_args.num_involver, -1)
                    obs, obs_critic, rewards, dones, infos = self.envs.step(
                        actions_e)
                ...
                available_actions = np.array(
                    [[None for agent_id in range(self.num_agents)] for info in infos])
            ...
            if self.all_args.training_bf:
                ...
                for i, train_info in enumerate(train_infos):
                    self.writter.add_scalars(
                        'training process_{}'.format(i), train_info, episode)
            ...
            if episode % self.log_interval == 0:
                ...
                for t in range(min(len(threads_rew), 5)):
                    ...
                    print("Reward for thread " + str(t+1) + ": " + str(rew) + " " + str(round(np.mean(rew), 2)) +
                          "  Inventory: " + str(inv)+"  Order: " + str(act) + " Demand: " + str(np.mean(threads_demand[t], 0)))
                rewards_log = []
                inv_log = []
                actions_log = []
                demand_log = []

        print("Training finished because of finish all the episodes: " +
    ...
    def warmup(self, train=True, normalize=True, test_tf=False):
        ...
        for agent_id in range(self.num_agents):
            ...
            self.buffer[agent_id].share_obs[0] = np.array(
    ...
    def eval_para(self, test_tf=False):
        ...
        for eval_step in range(self.episode_length):
            ...
            eval_actions = np.array(eval_actions_collector).reshape(
                n_eval_rollout_threads, self.all_args.num_involver, -1)
            # Obser reward and next obs
            eval_obs, eval_obs_critic, eval_rewards, eval_dones, eval_infos = envs.step(
                eval_actions)

            eval_available_actions = None

            overall_reward.append(np.mean(eval_rewards))
            rewards_log.append(eval_rewards.reshape(
                n_eval_rollout_threads, self.num_involver))
        ...
        rewards_arr = np.array(rewards_log).reshape(
            (self.episode_length, n_eval_rollout_threads, self.num_involver))

        for eval_index in range(min(3, n_eval_rollout_threads)):
            rewards_log = rewards_arr[:, eval_index, :]
            eval_returns = np.zeros(
                (self.episode_length + 1, self.num_involver), dtype=np.float32)
            eval_returns[-1] = 0
            for step in reversed(range(rewards_log.shape[0])):
                eval_returns[step] = eval_returns[step + 1] * \
                    self.all_args.gamma + rewards_log[step]
                for agent_id in range(self.num_agents):
                    ...
                    if self.trainer[agent_id]._use_popart or self.trainer[agent_id]._use_valuenorm:
                        est_V = self.trainer[agent_id].value_normalizer.denormalize(np.array([est_V]))[
                            0]
