...
class HAPPO():
    ...
    def ppo_update(self, sample, update_actor=True, just_reset=False):
        ...
        values, action_log_probs, dist_entropy = self.policy.evaluate_actions(share_obs_batch,
                                                                              ...
                                                                              active_masks_batch)
        # print(torch.isnan(action_log_probs).any())
        # print(torch.isinf(action_log_probs).any())

        # actor update
        imp_weights = torch.prod(torch.exp(action_log_probs - old_action_log_probs_batch),dim=-1,keepdim=True)
        #imp_weights = torch.exp(action_log_probs - old_action_log_probs_batch)

        clip_lb,clip_ub = self.clip_bound if self.clip_bound else (1.0 - self.clip_param, 1.0 + self.clip_param) 
        # print(torch.isinf(imp_weights).any())
        # print(torch.isnan(imp_weights).any())
