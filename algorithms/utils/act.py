from .distributions import Bernoulli, Categorical, DiagGaussian, DiagGaussianCentral, Actor_Beta
import torch
...
class ACTLayer(nn.Module):
    ...
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain, args=None):
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            if args.beta:
                self.action_out = Actor_Beta(inputs_dim, action_dim, use_orthogonal, gain, args)
            else:
                self.action_out = DiagGaussian(inputs_dim, action_dim, use_orthogonal, gain, args) if not args.central_controller else DiagGaussianCentral(inputs_dim, action_dim, use_orthogonal, gain, args)
    ...
    def forward(self, x, available_actions=None, deterministic=False):
        ...
        if self.mixed_action :
            ...
            # print(self.action_out.__class__.__name__)
            if self.action_out.__class__.__name__=='Categorical':
                actions = action_logits.mode() if deterministic else action_logits.sample()
            else:
            # actions = action_logits.mode(round_tf=True, bound=(torch.tensor([0],device='cuda'),torch.tensor([1000],device='cuda'))) if deterministic else action_logits.sample(round_tf=True, bound=(torch.tensor([0],device='cuda'),torch.tensor([1000],device='cuda')))
                actions = action_logits.mode(round_tf=True, bound=self.bound) if deterministic else action_logits.sample(round_tf=True, bound=self.bound)  
