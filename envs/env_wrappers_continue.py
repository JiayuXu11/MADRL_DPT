...
class SubprocVecEnv(object):
        ...
        for agent in range(self.num_agent):
            ...
            if all_args.action_type == 'multi_discrete' or all_args.action_type == 'central_multi_discrete' or all_args.action_type == 'central_discrete':
                for d in self.signal_action_dim:
                    # 订货可定0-60,transship 可-20 - 20
                    total_action_space.append(spaces.Discrete(d))
            # physical action space
            elif all_args.action_type == 'discrete':
                u_action_space = spaces.Discrete(
                    self.signal_action_dim)  # 5个离散的动作
                total_action_space.append(u_action_space)
            else:
                u_action_space = spaces.Box(low=-self.u_range, high=+self.u_range, shape=(
                    self.signal_action_dim,), dtype=np.float32)  # [-1,1]
                total_action_space.append(u_action_space)
    ...
    def reset(self, train=True, normalize=True, test_tf=False):
        results = [env.reset(train, normalize, test_tf)
...
class DummyVecEnv(object):
    def __init__(self, all_args):
        ...
        for agent in range(self.num_agent):
            ...
            self.observation_space.append(spaces.Box(
                low=-np.inf, high=+np.inf, shape=(self.signal_obs_dim,), dtype=np.float32))  # [-inf,inf]
            self.observation_space_critic.append(spaces.Box(low=-np.inf, high=+np.inf, shape=(self.signal_obs_critic_dim,),
                                                            dtype=np.float32))
    ...
    def reset(self, test_tf=False, normalize=True):
        results = [env.reset(train=False, test_tf=test_tf,
                             normalize=normalize) for env in self.env_list]
        obs, critic_obs = zip(*results)
        return np.stack(obs), np.stack(critic_obs)
...
class EvalVecEnv(object):
    def __init__(self, all_args, eval_tf=True):
        ...
        envs: list of gym environments to run in subprocesses
        """
        self.data_path = [all_args.eval_dir+'/{}/'.format(i) for i in range(all_args.num_involver)] if eval_tf else [
            all_args.test_dir+'/{}/'.format(i) for i in range(all_args.num_involver)]
    ...
    def reset(self, normalize=True):
        ...
        results = [env.reset(train=False, demand_list_set=self.eval_data[index],
                             normalize=normalize) for index, env in enumerate(self.env_list)]
        obs, critic_obs = zip(*results)
        return np.stack(obs), np.stack(critic_obs)
