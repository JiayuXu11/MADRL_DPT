...
class Env(object):
    ...
    def __init__(self, args):
        ...
        self.obs_dim = self.get_obs_dim(
            self.instant_info_sharing, self.actor_obs_step)
        # critic_obs
        self.use_centralized_V = args.use_centralized_V
        self.obs_critic_dim = self.get_critic_obs_dim(
            self.use_centralized_V, True)

        ACTION_DIM_DICT = {'discrete': (DEMAND_MAX*2+1)*(DEMAND_MAX+1), 'multi_discrete': [DEMAND_MAX*3+1, DEMAND_MAX*2+1], 'continue': 2, 'central_multi_discrete': [
            DEMAND_MAX*3+1, DEMAND_MAX*2+1]*self.agent_num, 'central_discrete': [(DEMAND_MAX*2+1)*(DEMAND_MAX+1)]*self.agent_num}
    ...
    def get_shipping_order(self, matrix):
        ...
        indices_sorted_by_value = np.unravel_index(
            indices_sorted_by_value, matrix.shape)

        # exclude diagonal elements and symmetric elements
        indices_sorted_by_value = list(
            zip(indices_sorted_by_value[0], indices_sorted_by_value[1]))
        indices_sorted_by_value = [
            index for index in indices_sorted_by_value if index[0] < index[1]]
    ...
    def get_training_data(self):
        ...
        if (self.generator_method == 'merton'):
            demand_list = [np.array(generator.merton(
                EPISODE_LEN, DEMAND_MAX).demand_list) for _ in range(self.agent_num)]
            ...
            demand_list = [generator.poisson(
                EPISODE_LEN, DEMAND_MAX/2, DEMAND_MAX).demand_list for _ in range(self.agent_num)]
            ...
            demand_list = [generator.normal(
                EPISODE_LEN, DEMAND_MAX/2, DEMAND_MAX/4, DEMAND_MAX).demand_list for _ in range(self.agent_num)]
    ...
    def get_obs_dim(self, info_sharing, obs_step):
        ...
        transship_dim_dict = {'no_transship': 0,
                              'self_transship': 1, 'all_transship': self.agent_num}
    ...
    def get_critic_obs_dim(self, info_sharing, obs_step):
        ...
        demand_info_num = (5 if 'quantile' in self.demand_info_for_critic else 0)+(
            self.lead_time if 'LT_all' in self.demand_info_for_critic else 0)+(1 if 'LT_mean' in self.demand_info_for_critic else 0)
    ...
    def reset(self, train=True, normalize=True, demand_list_set=None):
        ...
        self.order = [[S_O for i in range(self.lead_time)]
        ...
        sub_agent_obs = self.get_step_obs(
            self.instant_info_sharing, self.actor_obs_step)
    ...
    def step(self, actions):
        ...
        # Map outputs of MADRL to actual ordering actions
        action = self.action_map(actions)

        # reward = self.state_update(action) # System state update
        reward = self.state_update_transship_revenue_sharing(action)

        sub_agent_obs = self.get_step_obs(
            self.instant_info_sharing, self.actor_obs_step)  # Get step obs
        sub_agent_reward = self.get_processed_rewards(
            reward)  # Get processed rewards
    ...
    def action_map(self, actions):
        ...
        if self.action_type == 'discrete' or self.action_type == 'central_discrete':
            order_amounts = [i[0] for i in actions]
            transship_amounts = [i[1] for i in actions]
            ...
            for i in range(self.agent_num):
                ...
                transship_amount = max(
                    action[i][1]-DEMAND_MAX, -self.inventory[i]-self.order[i][0])
            ...
            for i in range(self.agent_num):
                ...
                transship_amounts.append(
                    max(action[i][1], -self.inventory[i]-self.order[i][0]))
        ...
        transship_amounts = [0 for i in range(len(actions))]
        if self.homo_distance:
            self.transship_merge_homo(transship_amounts, order_amounts)
        elif self.mini_pooling["flag"]:
            self.transship_merge_mp(
                transship_amounts, threshold=self.mini_pooling['threshold'], how=self.mini_pooling['how'])
        else:

            look_ahead_days = min(self.lead_time, 8) + 1
            info = self.get_info()
            next_demand = [int(ele['demand']) for ele in info]
            next_start_inventory = [-int(ele['shortage'])
                                    if ele['shortage'] <= 0 else 0 for ele in info]
            next_orders = [int(self.order[i][0])
                           for i in range(self.agent_num)]
            next_shortage = [int(next_orders[i] + next_start_inventory[i] -
                                 next_demand[i]) for i in range(self.agent_num)]

            model = pulp.LpProblem("best_transship", pulp.LpMinimize)

            orders = []
            for i in range(self.agent_num):
                curr_orders = [int(self.order[i][day]) for day in range(
                    self.lead_time)]+[int(order_amounts[i])]
                orders.append(curr_orders)
            I = list(range(self.agent_num))
            J = list(range(self.agent_num))
            T = list(range(look_ahead_days))

            # add variables
            tran = [[[pulp.LpVariable(f'tran_{i}_{j}_{t}', 0) for t in range(len(T))] for j in range(len(J))] for i in range(len(I))]
            inv = [[pulp.LpVariable(f'inv_{i}_{t}', 0) for t in range(len(T))] for i in range(len(I))]
            s_inv = [[pulp.LpVariable(f's_inv_{i}_{t}', 0) for t in range(len(T))] for i in range(len(I))]
            s = [[pulp.LpVariable(f's_{i}_{t}', 0) for t in range(len(T)-1)] for i in range(len(I))]

            for i in I:

                model += s_inv[i][0] == next_start_inventory[i], f"Initial_Inventory_Constraint_{i}"

            for t in range(0, len(T)-1):
                for i in I:
                    model += inv[i][t+1] == s_inv[i][t] - pulp.lpSum(tran[i][j][t] for j in range(len(J))) + pulp.lpSum(tran[j][i][t] for j in range(len(J))) + orders[i][t], f"Inventory_Update_Constraint_{i}_{t}"
        
                    model += s_inv[i][t+1] == inv[i][t+1] - s[i][t], f"Updated_s_inv_Constraint_{i}_{t}"
                    
                    model += s[i][t] <= inv[i][t+1], f"s_Limit_Constraint_1_{i}_{t}"
                    model += s[i][t] <= next_demand[i], f"s_Limit_Constraint_2_{i}_{t}"
                    
                    model += pulp.lpSum(tran[i][j][t] for j in range(len(J))) <= s_inv[i][t] + orders[i][t], f"tran_Sum_Constraint_{i}_{t}"

            it_tulist = [(i, j) for i in range(len(I)) for j in range(len(J))]

            for it in itertools.product(I, T):
                it_tulist.add(it)

            tran_cost_all = [sum([tran[i, j, t]*self.distance[i][j]*self.shipping_cost_per_distance for i in range(
                self.agent_num) for j in range(self.agent_num)])*self.gamma**t for t in T]

            s_it_list = [ele for ele in [(self.R + self.H - self.C + self.P) *
                                         self.gamma**t for t in range(len(T))] for i in range(self.agent_num)]
            h_it_list = [ele for ele in [
                (self.H)*self.gamma**t for t in range(len(T))] for i in range(self.agent_num)]
            p_it_list = [ele for ele in [
                (self.P)*next_demand[i]*self.gamma**t for t in range(len(T))] for i in range(self.agent_num)]

            p_d = sum(p_it_list)

            cost_s = dict(zip(it_tulist, s_it_list))
            cost_h = dict(zip(it_tulist, h_it_list))

            objective_value = (
                pulp.lpSum(s[i][t] * cost_s[i][t] for i in range(len(I)) for t in range(len(T)-1))
                - pulp.lpSum(s_inv[i][t] * cost_h[i][t] for i in range(len(I)) for t in range(len(T)))
                - p_d
                - sum(tran_cost_all)
            )
            model += objective_value, "Objective"

            solver = pulp.COIN_CMD(maxSeconds=60)
            ...
            if model.status != pulp.LpStatusOptimal:
                ...
                print(info)
                print(next_start_inventory, next_demand, orders)
            ...
            if model.status == pulp.LpStatusOptimal:
                ...
                for var in model.variables():
                    ...
                    if len(var.name) >= 4:
                        if var.name[0:4] == "tran" and var.name[-2] == "0":
                            # print(int(var.name[-6]),int(var.name[-4]))
                            s = var.name
                            s = s.replace("tran(", "").replace(")", "")
                            ind_i, ind_j, _ = map(int, s.split(","))
            ...
            for a1 in range(self.agent_num):
                for a2 in range(self.agent_num):
                    if transship_amounts[a1]*transship_amounts[a2] < 0:
                        tran_amount = min(
                            abs(transship_amounts[a1]), abs(transship_amounts[a2]))
                        self.transship_matrix[a1][a2] = tran_amount if transship_amounts[a1] > 0 else -tran_amount
                        self.transship_matrix[a2][a1] = - \
                            self.transship_matrix[a1][a2]
    ...
    # homo distance下的撮合机制
    def transship_merge_homo(self, transship_amounts, order_amounts):
        # 撮合transship
        ...
        next_start_inventory = [-int(ele['shortage'])
                                if ele['shortage'] <= 0 else 0 for ele in info]
        next_orders = [int(self.order[i][0]) for i in range(self.agent_num)]
        next_shortage = [int(next_orders[i] + next_start_inventory[i] -
                             next_demand[i]) for i in range(self.agent_num)]
    ...
    def transship_merge_mp(self, transship_amounts, threshold, how):
        ...
        def mini_pooling(distance_matrix, transship_amounts, thres):
            ...
            mini_pool_pairs = [
                pair[:2] for pair in possible_pairs if pair[2] - possible_pairs[0][2] <= thres]
        ...
        while True:
            mini_pool_pairs = mini_pooling(
                self.distance, transship_amounts, threshold)
            if not mini_pool_pairs:
                break
            while mini_pool_pairs:
                ...
                if how == 'even':
                    ...
                    weights = [transship_amounts[pair[1]]
                               for pair in mini_pool_pairs]
                    curr_pair = random.choice(
                        mini_pool_pairs, weights=weights, k=1)
                ...
                if transship_amounts[curr_pair[0]] == 0:
                    mini_pool_pairs = list(
                        filter(lambda x: curr_pair[0] not in x, mini_pool_pairs))
                if transship_amounts[curr_pair[1]] == 0:
                    mini_pool_pairs = list(
                        filter(lambda x: curr_pair[1] not in x, mini_pool_pairs))
    ...
    def get_step_obs(self, info_sharing, obs_step, demand_today=True):
        ...
        for i in range(self.agent_num):
            ...
            if info_sharing:
                if i == 0:
                    base_arr = np.array(self.inventory + ([(self.demand_list[k][self.step_num-1] if self.step_num >
                                        0 else DEMAND_MAX/2) for k in range(self.agent_num)] if demand_today else []))
                    order_arr = np.array(order_all)
            else:
                base_arr = np.array([self.inventory[i], (self.demand_list[i][self.step_num-1] if self.step_num >
                                    0 else DEMAND_MAX/2)]) if demand_today else np.array([self.inventory[i]])
            ...
            if self.obs_transship == 'no_transship' or info_sharing:
                ...
                transship_arr = np.array(
                    self.transship_intend + self.transship_request)
            elif self.obs_transship == 'self_transship':
                transship_arr = np.array(
                    [self.transship_intend[i], self.transship_request[i]])
            else:
    ...
    def set_demand_statistics(self):
        if self.step_num == 0:
            self.demand_dy_sorted = [np.sort(demand[min(self.step_num+self.lead_time, self.episode_max_steps):min(
                self.looking_len+self.step_num+self.lead_time, self.episode_max_steps)]) for demand in self.demand_list]
            self.demand_LT = [[(demand[idx] if idx < self.episode_max_steps else 0)for idx in range(
                self.step_num, self.step_num+self.lead_time)] for demand in self.demand_list]
            ...
            for agent in range(self.agent_num):
                del_num = self.demand_list[agent][min(
                    self.step_num+self.lead_time-1, self.episode_max_steps-1)]
                insert_num = self.demand_list[agent][insert_index] if insert_index < self.episode_max_steps else self.demand_q50[agent]
                self.demand_dy_sorted[agent] = self.del_and_insert(
                    self.demand_dy_sorted[agent], del_num, insert_num)

                del_num_LT = self.demand_list[agent][self.step_num-1]
                insert_num_LT = self.demand_LT[agent][-1]
                self.demand_LT_mean[agent] = self.demand_LT_mean[agent] + \
                    (-del_num_LT+insert_num_LT)/self.lead_time

        self.demand_q5 = [demand[int(0.05*(len(demand)-1))]
                          for demand in self.demand_dy_sorted]
        self.demand_q25 = [demand[int(0.25*(len(demand)-1))]
                           for demand in self.demand_dy_sorted]
        self.demand_q50 = [demand[int(0.5*(len(demand)-1))]
                           for demand in self.demand_dy_sorted]
        self.demand_q75 = [demand[int(0.75*(len(demand)-1))]
                           for demand in self.demand_dy_sorted]
        self.demand_q95 = [demand[int(0.95*(len(demand)-1))]
                           for demand in self.demand_dy_sorted]
    ...
    def get_LT_inv_shortage(self):
        ...
        for agent in range(self.agent_num):
            for lt in range(self.lead_time):
                inv = self.LT_inv[agent]+self.order[agent][lt] - \
                    self.demand_list[agent][min(
                        lt+self.step_num, self.episode_max_steps-1)]
                self.LT_inv[agent] = max(inv, 0)
                self.LT_avg_shortage[agent] += -min(inv, 0)
            self.LT_avg_shortage[agent] = self.LT_avg_shortage[agent] / \
                self.lead_time
        ...
        for i in range(self.agent_num):
            ...
            if info_sharing:
                if i == 0:
                    ...
                    demand_quantile_arr = (self.demand_q5+self.demand_q25+self.demand_q50+self.demand_q75 +
                                           self.demand_q95) if 'quantile' in self.demand_info_for_critic else []
                    ...
                    demand_LT_arr = (self.demand_LT_all_helper if 'LT_all' in self.demand_info_for_critic else [
                    ]) + (self.demand_LT_mean if 'LT_mean' in self.demand_info_for_critic else [])
                ...
                # demand_mean_arr = np.array(demand_mean_arr)
                demand_quantile_arr = [self.demand_q5[i], self.demand_q25[i], self.demand_q50[i],
                                       self.demand_q75[i], self.demand_q95[i]] if 'quantile' in self.demand_info_for_critic else []
                demand_quantile_arr = np.array(demand_quantile_arr)
                ...
                # demand_std_arr = np.array(demand_std_arr)
                demand_LT_arr = (self.demand_LT[i] if 'LT_all' in self.demand_info_for_critic else [
                ]) + ([self.demand_LT_mean[i]] if 'LT_mean' in self.demand_info_for_critic else [])
                demand_LT_arr = np.array(demand_LT_arr)
            ...
            if (self.normalize):
                ...
                arr = np.concatenate([actor_arr, demand_quantile_arr*2/DEMAND_MAX-1,
                                     demand_LT_arr*2/DEMAND_MAX-1, other_actor_arr*2/DEMAND_MAX-1])
            else:
                # arr = np.concatenate([actor_arr,demand_mean_arr, demand_std_arr, demand_quantile_arr, demand_LT_arr])
                arr = np.concatenate(
                    [actor_arr, demand_quantile_arr, demand_LT_arr, other_actor_arr])
            sub_agent_obs.append(arr)
    ...
    def get_processed_rewards(self, reward):
        ...
        processed_rewards = [
            [self.alpha*i + (1-self.alpha)*np.mean(reward)] for i in reward]
        self.reward = [self.alpha*i +
                       (1-self.alpha)*np.mean(reward) for i in reward]
        self.reward_cum = [self.reward_cum[i]+self.reward[i]
                           for i in range(self.agent_num)]
        return processed_rewards

    # transship的收益
    def get_transship_revenue(self, action):
        cur_demand = [self.demand_list[i][self.step_num]
                      for i in range(self.agent_num)]
        ...
        for i in range(self.agent_num):
            ...
            shortage_wihout_transship = max(
                cur_demand[i]-inv_start_without_transship, 0)
            transship_revenue[i] = max((-self.C+self.P)*min(shortage_wihout_transship, ts) -
                                       self.S*ts - self.H*max(ts-shortage_wihout_transship, 0), 0)
    ...
    def state_update_transship_revenue_sharing(self, action):
        ...
        for i in range(self.agent_num):
            ...
            inv_start = self.inventory[i] + \
                self.order[i][0]+self.transship_request[i]
            self.inventory_start[i] = inv_start

            revenue_demand = cur_demand[i] * \
                self.R if 'reward' in self.reward_type else 0
            norm_drift = cur_demand[i] * \
                self.reward_norm_multiplier if 'norm' in self.reward_type else 0

            # 纯运费
            self.shipping_cost_pure[i] = sum([self.shipping_cost_matrix[i][j]*self.transship_matrix[i]
                                             [j] if self.transship_matrix[i][j] > 0 else 0 for j in range(self.agent_num)])
            # 运费+买卖货的费用
            self.shipping_cost_all[i] = self.shipping_cost_pure[i] + \
                self.C*(action[i][1])

            self.ordering_cost[i] = (self.C*(action[i][0])+FIXED_COST*(1 if action[i][0] > 0 else 0)) if self.pay_first else (
                self.C*(self.order[i][0])+FIXED_COST*(1 if self.order[i][0] > 0 else 0))
            self.ordering_times[i] += (1 if action[i][0] > 0 else 0) if self.pay_first else (
                1 if self.order[i][0] > 0 else 0)
            self.penalty_cost[i] = -self.P*min(inv_start-cur_demand[i], 0)
            self.holding_cost[i] = self.H*max(inv_start-cur_demand[i], 0)
            # transship 后的reward
            reward = -self.ordering_cost[i]-self.shipping_cost_all[i] - \
                self.holding_cost[i]-self.penalty_cost[i] + \
                revenue_demand+norm_drift

            # transship前的reward
            reward_before = -self.ordering_cost[i]-self.H*max(inv_start_before-cur_demand[i], 0)+self.P*min(
                inv_start_before-cur_demand[i], 0)+revenue_demand+norm_drift
            ...
            if self.step_num > EPISODE_LEN-1:
                ...
                reward_before = reward_before + \
                    (self.C)*max(inv_start-cur_demand[i], 0)
        ...
        for i in range(self.agent_num):
            ...
            if self.transship_revenue_method == 'constant':
                transship_reallocate = -self.constant_transship_revenue * \
                    self.transship_request[i]
            else:
