...
class Env(object):
    ...
    def get_orders(self):
        ...
        # return self.order
        return [[t[0][0],t[0][1],t[0][2],t[0][3],t[1]] for t in zip(self.order,self.transship_request)]
    ...
    def action_map(self, action,one_hot=True):
        ...
        for i in range(len(action)):
            order_amounts.append(action[i])
            transship_amounts.append(0)
    ...
    def get_reset_obs(self,normalize):
        ...
        for i in range(self.agent_num):
            if(normalize):
                arr = np.array([self.inventory[i],self.demand_list[i][self.step_num-1], self.step_num]+self.order[i])/(DEMAND_MAX)
            else:
                arr = np.array([self.inventory[i],self.demand_list[i][self.step_num-1], self.step_num]+self.order[i])
