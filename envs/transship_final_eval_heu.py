...
class Env(object):
    ...
    def action_map(self, actions):
        ...
        if self.homo_distance:
            ...
            env = SOLVER
            model = env.createModel('best_transship')
            ...
            # add variables
            tran = model.addVars(len(I), len(J), len(T),
                                 lb=0, nameprefix='tran')

            inv = model.addVars(len(I), len(T), lb=0, nameprefix='inv')
            s_inv = model.addVars(len(I), len(T), lb=0, nameprefix='s_inv')
            s = model.addVars(len(I), len(T)-1, lb=0, nameprefix='s')
            ...
            for t in range(0, len(T)-1):
                for i in I:
                    model.addConstr(
                        inv[i, t+1] == s_inv[i, t] - tran.sum(i, "*", t) + tran.sum("*", i, t) + orders[i][t])
                    model.addConstr(s_inv[i, t+1] == inv[i, t+1] - s[i, t])
                    model.addConstr(s[i, t] <= inv[i, t+1])
                    model.addConstr(s[i, t] <= next_demand[i])

                    model.addConstr(tran.sum(i, "*", t) <=
                                    s_inv[i, t] + orders[i][t])
            ...
            model.setObjective(s.prod(cost_s) - s_inv.prod(cost_h) -
                               p_d - sum(tran_cost_all), COPT.MAXIMIZE)

            model.setParam(COPT.Param.TimeLimit, 60.0)
            # solve model
            model.solve()
    ...
    def transship_merge_homo(self, transship_amounts, order_amounts):
        ...
        # add variables
        tran = model.addVars(len(I), len(J), len(T), lb=0, nameprefix='tran')
        ...
        for t in range(0, len(T)-1):
            for i in I:
                model.addConstr(inv[i, t+1] == s_inv[i, t] - tran.sum(i,
                                "*", t) + tran.sum("*", i, t) + orders[i][t])
                model.addConstr(s_inv[i, t+1] == inv[i, t+1] - s[i, t])
                model.addConstr(s[i, t] <= inv[i, t+1])
                model.addConstr(s[i, t] <= next_demand[i])
