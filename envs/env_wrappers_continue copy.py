...
class SubprocVecEnv(object):
    def __init__(self, all_args):
        ...
        self.env_list = [Env(1.0 if all_args.central_controller else all_args.alpha,all_args.ratio_transship) for i in range(all_args.n_rollout_threads)]
...
class DummyVecEnv(object):
    def __init__(self, all_args):
        ...
        self.env_list = [Env(1.0 if all_args.central_controller else all_args.alpha,all_args.ratio_transship) for i in range(1)]
    ...
    def reset(self, test_tf=False):
        results = [env.reset(train = False,test_tf = test_tf) for env in self.env_list]
        obs, critic_obs = zip(*results)
        return np.stack(obs), np.stack(critic_obs)
